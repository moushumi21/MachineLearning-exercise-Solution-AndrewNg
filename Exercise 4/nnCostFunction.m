...
regularization = lambda/(2*m) * (sum(sum(Theta1_new.^2)) + sum(sum(Theta2_new.^2)));

J = J +regularization;

del3 = a3 - y_new; %5000*10
del2 = (del3 * Theta2).* [ones(size(z2,1),1) sigmoidGradient(z2)];
del2 = del2(:, 2:end);%5000*25
Theta2_grad = (del3'*a2)/m; %10*26
Theta1_grad = (del2'*a1)/m; %25*401
    
%regularization
reg_theta1_grad = (lambda/m)*[zeros(size(Theta1,1) ,1) Theta1_new];
reg_theta2_grad = (lambda/m)*[zeros(size(Theta2,1) ,1) Theta2_new];
